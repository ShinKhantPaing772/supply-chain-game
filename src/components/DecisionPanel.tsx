...
export function DecisionPanel({ game, onChange }: Props) {
  ...
  return <aside className="decisionPanel simplifiedPanel">
    ...
    <div className="panelScroll simplifiedScroll">
      ...
      <section className="simpleSection"><div className="simpleHeading"><span>{PRODUCT.shortName} core plan</span><small>Available from January</small></div><RangeControl label="Component kits to purchase" value={totalProcurement} max={180} icon={PackagePlus} onChange={setTotalProcurement} /><RangeControl label={`${PRODUCT.shortName} speakers to produce`} value={decision.regularProduction} max={80} icon={Gauge} onChange={setProduction} /></section>
